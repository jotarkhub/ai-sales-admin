...
| WhatsApp — FollowUp Dispatch (kirim follow-up jatuh tempo) | `IMPLEMENTED_UNVERIFIED` | `MessageTemplateResolver` (ambil teks dari Business::message_templates, dengan alias), `FollowUpDispatchService` (cek ulang consent/opt-out, kirim lewat provider, retry dengan backoff 15 menit sampai max_attempts lalu gagal permanen), command `follow-ups:send` + scheduler tiap menit. 9 test baru (termasuk skenario retry & gagal permanen pakai provider palsu yang sengaja gagal). Lolos `php -l`, **menunggu `php artisan test` sungguhan** |

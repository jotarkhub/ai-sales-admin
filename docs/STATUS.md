...
| WhatsApp — Webhook Receiver (Fase 4a, terima pesan/status masuk) | `IMPLEMENTED_UNVERIFIED` | `VerifyWhatsAppWebhookSignature` (HMAC X-Hub-Signature-256 pakai App Secret), `WhatsAppWebhookController` (GET handshake verifikasi + POST terima event), `WhatsAppWebhookService` (idempotency lewat `webhook_events`, auto-buat Lead untuk nomor baru, simpan Message inbound, update MessageStatus dari delivery receipt, tidak pernah crash 500 ke Meta — kegagalan per-item dicatat, bukan bikin retry storm). Belum memicu balasan AI (itu Conversation Engine, Fase 4b). 12 test baru. Lolos `php -l`, **menunggu `php artisan test` sungguhan** |
| AI — Conversation Engine (Fase 4b: prompt building, knowledge retrieval, scoring, eskalasi, balas otomatis) | belum dimulai | `CREDENTIAL_REQUIRED` (API key OpenAI belum ada — provider abstraction + webhook receiver di atas sudah siap dipakai begitu kredensial tersedia) |
